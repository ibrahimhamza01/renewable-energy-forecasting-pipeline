"""
Smart EDA on raw NOAA ISD data for wind forecasting project.

Reads 1 year of data across 20 curated stations (CA/TX/MN/FL),
parses the WND field using src/parsing/parse_wnd.py, and produces
publication-ready plots in outputs/figures/.

Run:
  spark-submit ... scripts/eda_wind_noaa.py
"""
...
# 5 stations per state — mix of airports + coastal/inland for good diversity.
...
STATIONS = [
    ...
    ("CA", "72389023155", "Palmdale Regional"),
    ("CA", "72492023230", "Oakland INTL"),
    # Texas
    ("TX", "72243012960", "Houston Hobby"),
    ("TX", "72259013960", "Dallas Love Field"),
    ("TX", "72253012921", "San Antonio INTL"),
    ("TX", "72261722530", "Del Rio INTL"),
    ("TX", "72266023042", "Lubbock INTL"),
    # Minnesota
    ("MN", "72658014922", "Minneapolis-St Paul INTL"),
    ("MN", "72655014918", "Duluth INTL"),
    ("MN", "72657014925", "St Cloud Regional"),
    ("MN", "72756094931", "International Falls"),
    ("MN", "72655314914", "Rochester INTL"),
    # Florida
    ("FL", "72202012839", "Miami INTL"),
    ("FL", "72205012815", "Orlando INTL"),
    ("FL", "72211012842", "Tampa INTL"),
    ("FL", "72206013889", "Jacksonville INTL"),
    ("FL", "72203712844", "Fort Lauderdale"),
]
...
def main():
    ...
    # Validate which station files actually exist for this year
    ...
    for state, sid, name in STATIONS:
        ...
        if fs.exists(Path(p)):
            valid_stations.append((state, sid, name))
        else:
            missing_stations.append((state, sid, name))

    if missing_stations:
        print(f"WARNING: {len(missing_stations)} stations NOT found in {YEAR}:")
        for state, sid, name in missing_stations:
            print(f"   {state}  {sid}  {name}")

    if not valid_stations:
        raise RuntimeError("No valid station files found")

    print(f"OK: {len(valid_stations)} stations validated")
    ...
    print("=" * 70)
    print(f"Reading {YEAR} data for {len(STATIONS)} stations across CA/TX/MN/FL")
    print("=" * 70)
    ...
    # -------------------------------------------------------------------------
    # Metric aggregations (Spark → pandas, tiny results)
    # -------------------------------------------------------------------------
    metrics = {"year": YEAR, "total_rows": int(total_rows), "n_stations": len(STATIONS)}

    # ---- 1. Data quality: records, nulls, coverage per station
    ...
    # ---- 3. Raw wind speed sample per state (for histograms)
    ...
    # ---- 6. Wind rose per state (direction + speed)
    ...
    # -------------------------------------------------------------------------
    # Plotting (pandas + matplotlib, single machine)
    # -------------------------------------------------------------------------
    STATE_COLORS = {"CA": "#1f77b4", "TX": "#d62728", "MN": "#2ca02c", "FL": "#ff7f0e"}
    ...
    # --- Plot 2: Wind speed distribution per state ---
    ...
    for ax, state in zip(axes, states):
        ...
        ax.axvline(stats["mean"], color="black", linestyle="--", linewidth=1.5, label=f"mean={stats['mean']:.1f}")
        ax.set_title(f"{state} — μ={stats['mean']:.1f} m/s, p95={stats['p95']:.1f}", fontweight="bold")
        ax.set_xlabel("Wind speed (m/s)")
        ax.set_xlim(0, 25)
        ax.legend()
        ax.grid(alpha=0.3)
    axes[0].set_ylabel("Frequency")
    fig.suptitle(f"Wind speed distribution by state — {YEAR}", fontsize=14, fontweight="bold", y=1.02)
    ...
    for state in states:
        ...
        ax.plot(d["hour"], d["mean_speed"], marker="o", label=state, color=STATE_COLORS[state], linewidth=2)
    ...
    for state in states:
        ...
        ax.plot(d["month"], d["mean_speed"], marker="o", label=state, color=STATE_COLORS[state], linewidth=2)
    ax.set_xlabel("Month")
    ax.set_ylabel("Mean wind speed (m/s)")
    ...
    for ax, state in zip(axes, states):
        ...
        if d.empty:
            ...
            continue
        pivot = d.pivot_table(index="dir_bin", columns="speed_bucket", values="count", fill_value=0)
    ...
    ax.set_title(f"Station locations — {len(locations)} stations", fontsize=13, fontweight="bold")
    ax.legend(title="State")
    ax.grid(alpha=0.3)
